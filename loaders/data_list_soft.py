...
def make_dataset_fromlist(image_list):
    f = torch.load(image_list)
    image_index = []
    label_list = []
    for idx in range(len(f)):
        image_index.append(f[idx][0])
        label_list.append(f[idx][1])
    """
...
class Imagelists_VISDA_Soft(object):
...
if __name__ == "__main__":
    make_dataset_fromlist('../data/txt/multi/pseudo_label/pseudo_label_sketch_painting_3.pt')