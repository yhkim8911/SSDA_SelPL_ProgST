...
from loaders.data_list_soft import Imagelists_VISDA_Soft
...
def return_dataset_feature(args):
    base_path = './data/txt/%s' % args.dataset
    root = './dataset/%s/' % args.dataset
    image_set_file_t = \
        os.path.join(base_path,
    ...
    target_dataset_unl = Imagelists_VISDA(image_set_file_unl, root=root,
                                          transform=data_transforms['test'])
    class_list = return_classlist(image_set_file_t)
    print("%d classes in this dataset" % len(class_list))
    if args.net == 'alexnet':
        bs = 32
    elif args.net == 'densenet':
    ...
    target_loader_unl = \
        torch.utils.data.DataLoader(target_dataset_unl,
                                    batch_size=bs * 2, num_workers=3,
                                    shuffle=False, drop_last=False)
    return target_loader_labeled, target_loader_unl, class_list


def return_dataset_soft_pl(args):
    base_path = './data/txt/%s' % args.dataset
    root = './dataset/%s/' % args.dataset
    image_set_file_s = \
        os.path.join(base_path,
    ...
    image_set_file_unl = \
        os.path.join(base_path,
                     ...
                     args.target + '_%d.txt' % (args.num))
    image_set_file_t_pl = \
        os.path.join(base_path, 'pseudo_label', args.net,
                     'pseudo_label_' + args.source + '_' +
                     args.target + '_' + str(args.num) + '_ratio_' + str(args.pl_ratio) + '.pt')
    ...
    target_dataset_test = Imagelists_VISDA(image_set_file_unl, root=root,
                                           transform=data_transforms['test'])
    target_dataset_pl = Imagelists_VISDA_Soft(image_set_file_t_pl, root=root,
                                              transform=data_transforms['val'])
    class_list = return_classlist(image_set_file_s)
    print("%d classes in this dataset" % len(class_list))
    if args.net == 'alexnet':
    ...
    target_loader_test = \
        torch.utils.data.DataLoader(target_dataset_test,
                                    ...
                                    shuffle=False, drop_last=False)
    target_loader_pl = \
        torch.utils.data.DataLoader(target_dataset_pl,
                                    batch_size=min(bs, len(target_dataset_pl)),
                                    num_workers=3,
                                    shuffle=True, drop_last=False)
    target_loader_pl_update = \
        torch.utils.data.DataLoader(target_dataset_pl,
                                    batch_size=min(1, len(target_dataset_pl)),
                                    num_workers=3,
                                    shuffle=False, drop_last=False)
    return source_loader, target_loader, target_loader_unl, \
        target_loader_val, target_loader_test, target_loader_pl, target_loader_pl_update, class_list
